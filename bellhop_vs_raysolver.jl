...
env = UnderwaterEnvironment(
         seasurface = Vacuum,
         seabed = SandyClay,
         ssp = SampledSSP([0.0, 1000.0, 4000.0], [1500.0, 1500.0, 1500.0], :linear),
         bathymetry = ConstantDepth(4000.0)
       )
#          ssp = SampledSSP([0.0, 1000.0, 4000.0], [1500.0, 1490.0, 1520.0], :linear),
...
begin
	...
	pm_bell = Bellhop(env, 1000, min_angle, max_angle, true, false);
	pm_ray = RaySolver(env; nbeams=1000, minangle=min_angle, maxangle=max_angle);
end

# ╔═╡ b44bbf8c-a34e-48b8-ac89-769db2627dd8
begin
	tx = AcousticSource(0.0, -50.0, 3000.0)
	nx, nz = 200, 200
	rx = AcousticReceiverGrid2D(1.0, 20000/nx, nx, -4000.0, 4000/nz, nz);
end

# ╔═╡ e7686715-ee72-4a18-b24a-8e59f4cb5365
begin
	x_bell = transmissionloss(pm_bell, tx, rx, mode=:coherent);
	x_ray = transmissionloss(pm_ray, tx, rx, mode=:coherent);
end

# ╔═╡ e903a1c8-7dbf-4222-ba1e-11eafded187b
plot(env; receivers=rx, transmissionloss=x_bell, clim=(-100, -30))

# ╔═╡ 9bd55275-6efa-474b-965f-f762ce6d1da5
plot(env; receivers=rx, transmissionloss=x_ray, clim=(-100, -30))

# ╔═╡ 41a79313-3c2b-45d5-8a69-54d871a34d51
# plot difference
plot(env; receivers=rx, transmissionloss=x_bell - x_ray)

# ╔═╡ a7abdfc7-41d9-48ea-a6bb-8676f49221cf
begin
	rx1D = AcousticReceiver(20000.0, -200.0)
...
git-tree-sha1 = "9f2fadc97c811d9f26c1c6295c7cc75062ec46f0"
...
git-tree-sha1 = "6c93aacf793327c8671f4b6a942355e0b76a8975"
