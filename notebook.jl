...
# ╔═╡ 47f7f4be-eb1b-11eb-02a3-9732b90ad4e0
begin
...
# ╔═╡ 65e7610d-6dc0-48fc-b145-c660a8225f98
md"Link to [UnderwaterAcoustics](https://org-arl.github.io/UnderwaterAcoustics.jl/dev/)"

# ╔═╡ 995710a2-9bac-4ff0-b06d-3ee965878786
@bind s Slider(0.9:.01:1.1)

# ╔═╡ 7626915d-604f-4557-b133-eeb7bb331d11
md"Define a slider that will scale sound celerity at 20 depth, current scaling = $s"

# ╔═╡ 273d1025-d8df-4d0c-b954-b926afb8ae17
rx = AcousticReceiver(100.0, -10.0);

# ╔═╡ 31e0f7a8-a1cb-4504-b3c5-ac63313931f5
tx = AcousticSource(0.0, -5.0, 1000.0);

# ╔═╡ 9e7e0abc-7f4f-4bc1-a101-8c9ed0acbfa9
env = UnderwaterEnvironment(
         seasurface = Vacuum,
         seabed = SandyClay,
         ssp = SampledSSP(0.0:20.0:40.0, [1500.0, s*1490.0, 1520.0], :smooth),
         bathymetry = ConstantDepth(40.0)
       )

# ╔═╡ 5885b1ee-2d45-4179-8d83-bccfbfefc468
#pm = RaySolver(env)
pm = Bellhop(env; gaussian=true)

# ╔═╡ 63047cc9-a776-4f37-bfe2-1f6334a16044
begin
	rx2d = AcousticReceiverGrid2D(1.0, 0.1, 100, -40.0, 0.2, 200)
	x = transmissionloss(pm, tx, rx2d, mode=:coherent)
end

# ╔═╡ e68f6449-f901-49b1-afe1-519a1d400a02
plot(env; receivers=rx2d, transmissionloss=x)

# ╔═╡ 0f1805c1-ef6b-46db-bef2-cc50840006e5
rx2d

# ╔═╡ a5711af1-0d19-4989-86b3-862dc9d0e7cf
r = eigenrays(pm, tx, rx)

# ╔═╡ 161c2f30-492e-4733-af8f-fa75182cf93c
begin
p1 = plot(env; sources=[tx], receivers=[rx], rays=r)
p2 = plot(ssp(env))
plot(p1, p2, layout = grid(1, 2, widths=[0.7 ,0.3]))
end

# ╔═╡ 7af72fec-cca4-47c1-ba28-0d710acf984d
plot(ssp(env))

# ╔═╡ d2493528-62a8-4104-9af7-3cf0bfe3d9dc
models()
...
git-tree-sha1 = "bad4640d622657238f1144da2a639ba1703c5352"
...
git-tree-sha1 = "0d4d020a75f000ef926f8721e3bd268f1a6355a9"
...
[[UnderwaterAcoustics]]
deps = ["Colors", "DSP", "DifferentialEquations", "DocStringExtensions", "FFTW", "ForwardDiff", "Interpolations", "LinearAlgebra", "Optim", "Printf", "RecipesBase", "Requires", "SignalAnalysis"]
git-tree-sha1 = "b33f62c649f1bcf86b5c6b687577e8c67ee24e45"
